...
<head>
    ...
    function GetUsers() {
        ...
        var ts = new Date();
        var url = "security_PriApprover_Details_Ajax.asp?f=GetUsers&id=" + currentDeptValue + "&gid=" + currentGroupValue + "&wid=" + currentWorkflowValue + "&ts=" + ts.getTime();
    ...
    function validateForm(sRefresh) {
		...
		if (numberUsers > 0)
		{
			//sTemp = $('rBtnUser').value + '';
			sTemp = currentSelectedUserID;
		}
		...
		var hdnGroupID = $('hdnGroupID');
		hdnGroupID.value = currentGroupValue;
		var hdnWorkflowID = $('hdnWorkflowID');
		hdnWorkflowID.value = currentWorkflowValue;
...
<form name="theForm" action="" method="POST" onSubmit="return false;" ID="theForm">
	...
	<input type="hidden" name="hdnWorkflowID" id="hdnWorkflowID" value="" />
</form>
