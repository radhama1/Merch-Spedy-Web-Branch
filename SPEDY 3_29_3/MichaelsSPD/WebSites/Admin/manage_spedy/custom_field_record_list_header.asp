...
<head>
	...
	function sortDetails(col_ordinal, sort_direction)
	{
		...
		parent.frames["DetailFrame"].document.location = "custom_field_record_list.asp?sort=" + col_ordinal + "&direction=" + sort_direction;
	}
...
<table cellpadding=0 cellspacing=0 border=0>
	<tr>
		...
		<%=writeSeparatorBar()%>
		<%=writeColHeader(2, "Record Type", true)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(0, "&nbsp;", false)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(4, "Modified", true)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(5, "Created", true)%>
