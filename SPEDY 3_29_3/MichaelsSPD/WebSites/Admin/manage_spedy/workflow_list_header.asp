...
<head>
	...
	function sortDetails(col_ordinal, sort_direction)
	{
		...
		parent.frames["DetailFrame"].document.location = "workflow_list.asp?sort=" + col_ordinal + "&direction=" + sort_direction;
	}
...
<table cellpadding="0" cellspacing="0" border="0">
	<tr>
		...
		<%'=writeSeparatorBar()%>
		<%=writeColHeader(1, "Name", true)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(0, "&nbsp;", false)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(2, "Description", true)%>
		<%=writeSeparatorBar()%>
