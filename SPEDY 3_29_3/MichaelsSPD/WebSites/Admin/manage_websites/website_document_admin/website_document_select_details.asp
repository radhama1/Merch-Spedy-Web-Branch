...
<head>
	...
	function uncheckSelectAll() {
		var oChk = parent.frames["DetailFrameHdr"].document.getElementById("chkSelectAll");
		oChk.checked = false;
		oChk = null;
	}
	//-->
		...
		'	Response.Write SQLStr
			...
			%>
				...
				<tr bgcolor=<%=rowcolor%> id="datarow" onDblClick="checkHighlight(true);" onClick="checkHighlight(false);" oncontextmenu="checkHighlight(true);">
					<td align="right" valign="middle"><input type=checkbox name="chkItem" value="num_<%=objRec("ID")%>_num" onclick="uncheckSelectAll()" align=left></td>
			...
			<script language="javascript">
			<!--
				parent.frames["DetailFrameHdr"].document.location = "website_document_select_details_header.asp?itemType=<%=thisElementType%>&cid=<%=categoryID%>&sort=<%=SortColumn%>&direction=<%=SortDirection%>";
				parent.frames["PagingNavFrame"].document.location = "./../../app_include/paging_navbar.asp?loc=<%=Request.ServerVariables("SCRIPT_NAME")%>&frm=DetailFrame&pageCount=<%=pageCount%>&curPage=<%=curPage%>&pageSize=<%=pageSize%>&q=<%=Server.URLEncode( "cid=" & Request("cid") & "&itemID=" & Request("itemID") & "&itemType=" & Request("itemType") )%>";
			//-->
			</script>
			<%
			else
