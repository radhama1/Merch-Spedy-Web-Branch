	...
	function initDataLayout(numCols)
	{
		...
		for (var i = 0; i <= numCols; i++)
		{
			...
			if (odataHeaderHandle && odataRowsetHandle)
			{
				...
				if (headercolWidth < datacolWidth) {msg+=" [resizeheader] "; headerCol_Resize(thisObjectName, datacolWidth);}
				if (headercolWidth > datacolWidth) {msg+=" [resizedata] "; dataCol_Resize(thisObjectName + "_data", headercolWidth);}
				if (headercolWidth > datacolWidth) {msg+=" [resizedataimg] "; dataCol_Resize(thisObjectName + "_dataimg", headercolWidth);}
