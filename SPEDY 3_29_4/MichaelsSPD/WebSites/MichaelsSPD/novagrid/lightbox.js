...
var Lightbox = {
	...
	hideAll: function(){
		var lboxes = document.getElementsByClassName('lbox')
		//lboxes.each(function(box){
		//		Element.hide(box)
		//	}
		//)
		for (var i = 0; i < lboxes.length; i++) {
			Element.hide(lboxes[i]);
        }
		if ($('overlay')){
...
Lightbox.base.prototype = {
	...
	initialize: function(element, options, frameid, loadpage){
		...
		if (frameid != null && loadpage != null)
		{
		    this.frame = $(frameid);
		    this.frame.src = loadpage;
		}
		else
		{
		    this.frame = null;
		...
		closer = '<img id="close" src="images/close.gif" alt="Close" title="Close this window" onclick="closeSettings();" />'
