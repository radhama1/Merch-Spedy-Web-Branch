...
if IsNumeric(SortColumn) and Trim(SortColumn) <> "" then
	...
	Session.Value("ValidationDoc_List_SortColumn") = SortColumn
else
	if IsNumeric(Session.Value("ValidationDoc_List_SortColumn")) and Trim(Session.Value("ValidationDoc_List_SortColumn")) <> "" then
		SortColumn = CInt(Session.Value("ValidationDoc_List_SortColumn"))
	else
		SortColumn = 0
		Session.Value("ValidationDoc_List_SortColumn") = SortColumn
	end if
...
if IsNumeric(SortDirection) and Trim(SortDirection) <> "" then
	...
	Session.Value("ValidationDoc_List_SortDirection") = SortDirection
else
	if IsNumeric(Session.Value("ValidationDoc_List_SortDirection")) and Trim(Session.Value("ValidationDoc_List_SortDirection")) <> "" then
		SortDirection = CInt(Session.Value("ValidationDoc_List_SortDirection"))
	else
		SortDirection = 0
		Session.Value("ValidationDoc_List_SortDirection") = SortDirection
	end if
...
SQLStr = "usp_Validation_Docs_ReturnAll " & SortColumn & ", " & SortDirection & ", " & pageSize & ", " & startRow
...
<head>
	...
	<!--
		...
		function showDetailsFrame(itemID)
		{
			...
			parent.parent.frames['DetailFrame'].document.location = "validation_rules_frm.asp?tid="+itemID;
			if (parent.parent.frames.document.getElementById('MainListFrame').rows == '*,0')
			{
				resizeFrame('200,*', 'MainListFrame', parent.parent.frames);
			}
		}
		
		function showReport(itemID)
		{
			var rptWin = window.open("./validation_admin/validation_rules_report.asp?tid=" + itemID, "valRptWindow_" + itemID, "width=750,height=550,toolbar=0,titlebar=1,location=0,directories=0,status=0,menuBar=0,scrollBars=1,resizable=1");
			rptWin.focus();
		}
		...
		<td width=100%>
			...
			<table cellpadding=0 cellspacing=0 onSelectStart="return false" width=100% border=0>
				...
				for rowCounter = 0 to displayCount - 1
					...
					Record_ID = SmartValues(arRecRows(dictRecCols("ID"), rowCounter), "CInt")
					Workflow_ID = SmartValues(arRecRows(dictRecCols("Workflow_ID"), rowCounter), "CInt")
					Workflow_Name = SmartValues(arRecRows(dictRecCols("Workflow_Name"), rowCounter), "CStr")
					Table_ID = SmartValues(arRecRows(dictRecCols("Metadata_Table_ID"), rowCounter), "CInt")
				%>
				<%if i > 0 then%>
				<tr bgcolor=e6e6e6><a name="datarow_<%=Record_ID%>" /><td colspan=100><img src="./images/spacer.gif" height=1 width=1></td></tr>
				<%end if%>
				<tr bgcolor=<%=rowcolor%> id="datarow_<%=Record_ID%>" onMouseOver="HoverRow(<%=Record_ID%>);" onMouseOut="HoverRow(0);" onDblClick="highlightRow();" oncontextmenu="HoverRow(<%=Record_ID%>);SelectRow(<%=Record_ID%>);displayMenu(); return false;">
					<td><img src="./images/spacer.gif" height=1 width=7></td>
					<td valign=top nowrap align=""><font style="font-family:Arial, Helvetica;font-size:11px;"><a href="javascript:showDetailsFrame(<%=Record_ID%>); void(0);"><%=Workflow_Name%></a></font></td>
					<td><img src="./images/spacer.gif" height=1 width=10></td>
					<td valign=top nowrap><font style="font-family:Arial, Helvetica;font-size:11px;"><a href="javascript:showDetailsFrame(<%=Record_ID%>); void(0);"><%=SmartValues(arRecRows(dictRecCols("Validation_Document"), rowCounter), "CStr")%></a></font></td>
					<td><img src="./images/spacer.gif" height=1 width=10></td>
					
					<td class="datacol"><a href="#<%=Record_ID%>" onMouseOver="hTaskBtn('taskIcon<%=Record_ID%>', true)"  onMouseOut="hTaskBtn('taskIcon<%=Record_ID%>', false)" onClick="SelectRow(<%=Record_ID%>);HoverRow(<%=Record_ID%>);this.parentElement.parentElement.click(); displayMenu(); return false;"><img id="taskIcon<%=Record_ID%>" src="./../app_images/tasks_icon_off.gif" width=24 height=16 alt=":: Click to access tasks ::" border=0></a></td>
					<td><img src="./images/spacer.gif" height=1 width=10></td>
			...
			<!--
				waitLyr.style.display = "none";
				parent.frames["DetailFrameHdr"].document.location = "validation_doc_list_header.asp?sort=" + <%=SortColumn%> + "&direction=" + <%=SortDirection%>;
...
<!--#include file="./include/validation_doc_list_contextmenu.asp"--><!--right click menu-->
</body>
</html>
<%
