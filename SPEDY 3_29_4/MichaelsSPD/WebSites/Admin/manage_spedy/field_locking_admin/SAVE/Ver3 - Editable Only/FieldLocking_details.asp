...
dim curFieldName, tblName, fieldName, userCatID, uiRow, strTemp, strTemp1, strLocked, titleID, dRow
...
<head>
	...
	<script language=javascript>
        ...
        function SetClearCat(obj,c) {
            // Set / Clear all Checkbox in a column and update its assoc hidden field
...
<!--border:1px solid dimgray;-->
    <div class="outer">
    <div style="overflow:hidden; width:100%; background-color:Silver; border-bottom-style: solid; border-bottom-width: 1px;	border-bottom-color: Gray;">
        <table border="0" cellspacing="0" cellpadding="2" id="tblTitle" style="padding:2px; width:96%; margin-left:1%; margin-right:3%;">
            <tr >
                <th class="colHeader1" width="<%=Col1 %>%">&nbsp;</th>
                <th class="colHeader1" align="right" width="<%=Col2 %>%">Set / Clear ALL Check boxes in Category</th>
            ...
            <tr >
                <th class="colHeader1" align="left" colspan="2" width="<%=Col1 %>%">Table / Field Name</th>
<!--                <th class="colHeader1" width="<%=Col2 %>%">&nbsp;</th>
-->                <%for c = 0 to numCols-1
                    if c=numCols-1 then
                        strTemp = "colHeader1"
                    else
                 ...
                 %>
                    <th class="<% =strTemp%>" align="center" id="C<%=c %>" width="<%=colWidth %>%"><%=SmartValues(aFieldCols(dictHeaderDataCols("Category"), c), "String") %></th>
                <%next ' c %>
            </tr>
        </table>
    </div>
<!--     style="overflow:auto; width:100%; height:480px;"
-->    <div class="inner">
        <table border="0" cellspacing="0" cellpadding="2" id="tblDetails" style="width:96%; margin-left:1%; margin-right:1%;" >
            <%
            uiRow = 1
            r = 0
            ' ======= FOR TESTING.   margin-left:1%; margin-right:1%;
            ' numRows = 6
            while r < numRows 
           	    ' Fields Returned: FieldLockingID, WorkFlowStageID, MetaDataColumnID, FLUserCatID, WorkFlowName, 
           	    '   WorkFlowStageName, TableName, FieldName, Locked
            ...
            %>
            <tr>
                <th class="rowHeader" style="text-align:left;" width="100%" colspan="<%=numCols+2 %>">
                    <%=tblName %>
                </th>
            </tr>
            <% end if  %>
