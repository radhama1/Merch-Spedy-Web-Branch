...
<head>
	...
	function sortDetails(col_ordinal, sort_direction)
	{
		...
		parent.frames["DetailFrame"].document.location = "order_details_status_details.asp?oid=" + Order_ID + "&sort=" + col_ordinal + "&direction=" + sort_direction;
	}
...
<table cellpadding=0 cellspacing=0 border=0>
	<tr>
		...
		<%=writeSeparatorBar()%>
		<%=writeColHeader(-1, "Activity&nbsp;Detail", false)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(1, "Previous&nbsp;Status", true)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(2, "Updated&nbsp;Status", true)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(3, "Updated&nbsp;By", true)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(4, "Activity&nbsp;Date", true)%>
