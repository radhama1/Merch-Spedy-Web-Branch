...
Class ASPUploadFromFlash
	...
	Public Sub SaveToDatabase(ByRef oField)
		...
		If IsObject(oField) Then
			For Each fileItem In UploadedFiles.Items
				'write the first file in the collection to the database.
				StreamRequest.Position = fileItem.Start
				oField.AppendChunk = StreamRequest.Read(fileItem.Length)
				Exit For
			Next
		End If
	End Sub
	...
	Public Sub Upload()
		...
		'Response.Write "vDataSep: " & String2Byte(vDataSep) & vbCrLf

		'Start of current separator
		nDataBoundPos = FindToken(vDataSep & Byte2String(vbCrLf & "Content-Disposition: form-data; name=""Filedata"""), 1)
		'Response.Write "nDataBoundPos: " & nDataBoundPos & vbCrLf

		'Beginning of last line
		nLastSepPos = FindToken(vDataSep & Byte2String(vbCrLf & "Content-Disposition: form-data; name=""Upload""" & vbCr), 1)
		'Response.Write "nLastSepPos: " & nLastSepPos & vbCrLf
		...
		Do Until nDataBoundPos = nLastSepPos
			...
			If nPosFile <> 0 And nPosFile < nPosBound Then
				...
				oUploadFile.Path = auxStr
				oUploadFile.FullPath = auxStr
				oUploadFile.ShortPath = Replace(auxStr, oUploadFile.FileName, "")
	...
	Private Function SkipToken(sToken, nStart)
		SkipToken = InstrBNew(nStart, VarArrayBinRequest, sToken)
		If SkipToken = 0 then
			Response.write "[SkipToken] Error in parsing uploaded binary request." & "<br>" & vbCrLf
			'Response.End
		end if
	...
	Private Function FindToken(sToken, nStart)
		FindToken = InstrBNew(nStart, VarArrayBinRequest, sToken)
	End Function

	Private Function ExtractField(sToken, nStart)
		Dim nEnd
		nEnd = InstrBNew(nStart, VarArrayBinRequest, sToken)
		If nEnd = 0 then
			Response.write "[ExtractField] Error in parsing uploaded binary request." & "<br>" & vbCrLf
			Response.End
		end if
