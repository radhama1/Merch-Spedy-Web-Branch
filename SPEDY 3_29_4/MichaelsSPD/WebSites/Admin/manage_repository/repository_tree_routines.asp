...
Function fnLoadXMLData(byRef objDocument, bLoadSubItems)
	...
	if objDocument is nothing then
		...
		If Err Then 
			...
			objRootNode.setAttribute("url") = "./repository_details.asp?cid=0"
		...
		If Err Then 
			...
			if not objRec.EOF then
				...
				Do Until objRec.EOF
					...
					objChildNode.setAttribute("url") = "./repository_details.asp?cid=" & objRec("ID")
...
Sub DisplayNode(ByVal objNodes, ByRef iElement, ByRef sLeftIndent, byRef sOpenFolders)
	...
	Dim oNode, sAttrValue, sNodeType, sURL, sNodeName, sCatID, sMode
	Dim NODE_ELEMENT
...
%><td height="16"><img src="../app_images/folderlist_icons/<%=fnChooseIcon(bIsLast, bIsRoot, sNodeType, bHasChildren, true)%>" width=31 height=16 border=0></td><td><img src="../app_images/folderlist_icons/pixel.gif" width=2 height=1></td><td nowrap class=node><img src="../app_images/folderlist_icons/pixel.gif" width=2 height=1><a href=<%=sURL%> target=DetailFrame onClick=objPreviousLink=fnSelectItem(this,objPreviousLink)><%=sAttrValue%></a><img src="../app_images/folderlist_icons/pixel.gif" width=2 height=1></td></tr></table><%
			else  'Otherwise this is a folder
%><%=vbCrLf%><table border=0 cellspacing=0 cellpadding=0><!-- Category_ID: <%=sCatID%> --><tr onMouseOver="HoverRow(<%=sCatID%>);" onMouseOut="HoverRow(-1);" onClick="SelectRow(<%=sCatID%>);" oncontextmenu="displayMenu(); return false;"><%
...
%><td height="16"><img onclick="doChangeTree(this, arClickedElementID, arAffectedMenuItemID);" class=LEVEL<%=iElement%> src="../app_images/folderlist_icons/<%=fnChooseIcon(bIsLast, bIsRoot, sNodeType, bHasChildren, bShowOpen)%>" id=<%=iElement%> width=31 height=16 border=0 name="<%=sMode%>"></td><td><img src="../app_images/folderlist_icons/pixel.gif" width=2 height=1></td><td nowrap class=node><img src="../app_images/folderlist_icons/pixel.gif" width=2 height=1><a href="<%=sURL%>" onMouseOver="window.status='';return true;" target=DetailFrame onClick="objPreviousLink=fnSelectItem(this,objPreviousLink)"><%=sAttrValue%></a><img src="../app_images/folderlist_icons/pixel.gif" width=2 height=1></td></tr></table><%
