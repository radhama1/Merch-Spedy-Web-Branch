...
<head>
	...
	<script language="javascript">
		...
		{
			parent.frames["DetailFrame"].document.location = "repository_details.asp?sort=" + col_ordinal + "&direction=" + sort_direction + "&<%=querystring%>";
		}
...
<table cellpadding=0 cellspacing=0 border=0>
	<tr>
		<td><img src="./images/spacer.gif" height=1 width=5></td>
		<%=writeColHeader(0, "Title", true)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(0, "&nbsp;", false)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(2, "File Name", true)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(9, "File Size", true)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(6, "Status", true)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(7, "Locked&nbsp;By", true)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(7, "Locked&nbsp;Since", false)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(8, "Published", true)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(3, "Modified", true)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(4, "Created", true)%>
		<%=writeSeparatorBar()%>
