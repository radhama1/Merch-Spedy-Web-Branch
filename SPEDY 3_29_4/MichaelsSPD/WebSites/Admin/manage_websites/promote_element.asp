...
Function promoteTree(byval thisID, byval boolIsTopLevel)
	...
	if boolIsTopLevel then
		if intPromoDirection = 1 then
			SQLStr = "sp_websites_promote_element " & CInt(thisID) & ", 1"
		else
			SQLStr = "sp_websites_promote_element " & CInt(thisID) & ", 0"
		end if
	...
	if boolPromoteChildren then
		...
		if not thisObjRec.EOF then
			Do Until thisObjRec.EOF
				...
				if intPromoDirection = 1 then
					SQLStr = "sp_websites_promote_element " & CInt(thisObjRec("Element_ID")) & ", 1"
				else
					SQLStr = "sp_websites_promote_element " & CInt(thisObjRec("Element_ID")) & ", 0"
				end if
...
<html>
<head>
	<title><%if intPromoDirection = 1 then%>Pro<%else%>De<%end if%>mote</title>
</head>
<body bgcolor="cccccc" topmargin=5 leftmargin=5 marginheight=5 marginwidth=5>

<form name="theForm" action="promote_element.asp" method=POST>
<table width=100% cellpadding=0 cellspacing=0 border=0>
	<tr>
		<td>
			<font style="font-family:Arial, Helvetica;font-size:16px;color:#000000">
			<b><%if intPromoDirection = 1 then%>Pro<%else%>De<%end if%>motion</b>
			</font>
		</td>
	</tr>
	<tr>
		<td>
			...
			<%if intPromoDirection = 1 then%>
			This action will copy the selected item from the Staging website to the Production/Live website.
			<%else%>
			This action will remove the selected item from the Production/Live website.
			<%end if%>
			</font>
		</td>
	</tr>
		...
		<td align=right>
			<table width=100% cellpadding=0 cellspacing=0 border=0>
				<tr>
					...
					<td nowrap>
						<font style="font-family:Arial, Helvetica;font-size:12px;color:#000000">
						<a href="javascript: void(0); document.theForm.chkIncludeChildren.click();" style="color:#000000;text-decoration:none;"><%if intPromoDirection = 1 then%>Pro<%else%>De<%end if%>mote all child elements</a>
						</font>
					</td>
		...
		<td align=right>
			<table width=100% cellpadding=0 cellspacing=0 border=0>
				<tr>
					...
					<td width=100%><img src="./../images/spacer.gif" height=1 width=5 border=0></td>
					<td align=right><input type=button name="btnCommit" value="    OK    " onClick="document.theForm.btnCommit.disabled=true;document.theForm.btnCommit.value='Working...';document.theForm.btnCancel.disabled=true;document.theForm.submit();"></td>
				</tr>
			</table>
		</td>
	</tr>
</table>
<input type=hidden name="promoswitch" value="<%=Request("promoswitch")%>">
