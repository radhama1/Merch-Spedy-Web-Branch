...
SQLStr = "sp_websites_admin_content_showstagingsorting_by_parentElementID '0" & Session.Value("websiteID") & "', '0" & parentCategoryID & "'"
'Response.Write "SQLStr = " & SQLStr & "<br>"
Call returnDataWithGetRows(connStr, SQLStr, arDetailsDataRows, dictDetailsDataCols)
%>
<html>
<head>
	<title>Sort Product Category</title>
	<style type="text/css">
	<!--
...
<table width=100% cellpadding=0 cellspacing=0 border=0>
	<form name="theForm" action="website_document_sort_save.asp" method="POST">
				...
				<tr>
					...
					<td valign=top>
						...
						for rowCounter = 0 to dictDetailsDataCols("RecordCount") - 1
							Display_Name = SmartValues(arDetailsDataRows(dictDetailsDataCols("Display_Name"), rowCounter), "CStr")
							thisID = SmartValues(arDetailsDataRows(dictDetailsDataCols("ID"), rowCounter), "CStr")
						Response.Write "<option value=""" & thisID & """>" & Display_Name & "</option>" & vbCrLf
						next
						%>
						</select>
					</td>
...
<script language="javascript">
	<!--
		parent.frames["header"].document.location = "website_document_sort_header.asp?pcid=<%=parentCategoryID%>";
		<%if dictDetailsDataCols("ColCount") > 0 and dictDetailsDataCols("RecordCount") > 0 then%>
		parent.frames["controls"].document.location = "website_document_sort_footer.asp?pcid=<%=parentCategoryID%>";
		<%end if%>
	//-->
</script>
