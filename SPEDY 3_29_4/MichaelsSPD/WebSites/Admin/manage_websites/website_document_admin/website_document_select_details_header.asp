...
<head>
	...
	function sortDetails(col_ordinal, sort_direction)
	{
		...
		parent.frames["DetailFrame"].document.location = "website_document_select_details.asp?itemType=<%=thisElementType%>&itemID=<%=thisElementID%>&cid=<%=CategoryID%>&sort=" + col_ordinal + "&direction=" + sort_direction;
	}
	function selectAllCheckboxes(bIsChecked) {
		var oDoc = parent.frames["DetailFrame"].document
		var oChks = oDoc.getElementsByTagName("INPUT")
		for (var i=0; i < oChks.length; i++)
		{
			if (oChks[i].type.toLowerCase() == "checkbox")
			{
				oChks[i].checked = bIsChecked;
			}
		}
		oDoc = null;
		oChks = null;
...
<table cellpadding=0 cellspacing=0 border=0>
	<tr>
		<td>
			<input type="checkbox" name="chkSelectAll" id="chkSelectAll" onclick="selectAllCheckboxes(this.checked);" style="display: inline;" />
