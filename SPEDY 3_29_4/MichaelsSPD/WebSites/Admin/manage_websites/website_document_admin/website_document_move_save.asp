...
Dim topicID, thisParentCatID, parentTopicID, sortTopicID
...
topicName = ""
topicID = checkQueryID(Request("tid"), 0)
sortTopicID = checkQueryID(Request("sortid"), 0)
parentTopicID = checkQueryID(Request("newParentID"), 0)
...
'	Response.Write "topicID: " & topicID & "<br>" & vbCrLf
'	Response.Write "sortTopicID: " & sortTopicID & "<br>" & vbCrLf
'	Response.Write "parentTopicID: " & parentTopicID & "<br>" & vbCrLf & "<br>" & vbCrLf & "<br>" & vbCrLf
...
if moveDirection = "within" then
	...
	'==============================================================================
	SQLStr = "UPDATE Website_Element_Data SET " &_
			" Parent_Element_ID = '0" & moveReference & "', " &_
			" SortOrder = 'ALPHA', " &_
			" Date_Last_Modified = '" & CDate(Now()) & "' " &_
			" FROM Website_Element_Data c " &_
			" INNER JOIN Website_Element_Promotion b ON b.Element_Data_ID = c.[ID] AND b.Promotion_State_ID = 1 " &_
			" WHERE c.Parent_Element_ID = '0" & parentTopicID & "' AND c.Element_ID = '0" & sortTopicID & "' "
...
'			" FROM Website_Element_Data c " &_
'			" WHERE c.Parent_Element_ID = '0" & parentTopicID & "' AND c.Element_ID <> '0" & sortTopicID & "' " &_
...
'			" ORDER BY c.SortOrder, c.Element_ID "
	...
	SQLStr = "SELECT c.[ID] As Element_Data_ID, c.[Element_ID], c.[Parent_Element_ID], c.SortOrder " &_
			" FROM Website_Element_Data c " &_
			" INNER JOIN Website_Element_Promotion b ON b.Element_Data_ID = c.[ID] AND b.Promotion_State_ID = 1" &_
			" WHERE c.Parent_Element_ID = '0" & parentTopicID & "' AND c.Element_ID <> '0" & sortTopicID & "' " &_
			" ORDER BY c.SortOrder, c.Element_ID "
	...
	if not objRec.EOF then
		Do Until objRec.EOF
			...
			if CInt(objRec("Element_ID")) = CInt(moveReference) then
			...
			SQLStr = "UPDATE Website_Element_Data SET " &_
					...
					" Date_Last_Modified = '" & CDate(Now()) & "' " &_
					"WHERE [ID] = '0" & objRec("Element_Data_ID") & "' "
	...
	'==============================================================================
	SQLStr = "UPDATE Website_Element_Data SET " &_
			" Parent_Element_ID = '0" & parentTopicID & "', " &_
...
if objConn.Errors.Count < 1 and Err.number < 1 then
	...
	SQLStr = "Select Top 1 Element_ShortTitle From Website_Element_Data Where Element_ID = " & sortTopicID
	...
	ActivityLog.Activity_Summary = "Moved Document " & topicName
	ActivityLog.Reference_Type = ActivityReferenceType.Websites_Document
	ActivityLog.Reference_ID = sortTopicID	
	ActivityLog.Save
...
'	Dim newNavString
'	SQLStr = "sp_websites_admin_climbladder " & sortTopicID & ", " & CInt(Session.Value("websiteID")) & ", 1"
'	Set objRec = objConn.Execute(SQLStr)
'	if not objRec.EOF then
