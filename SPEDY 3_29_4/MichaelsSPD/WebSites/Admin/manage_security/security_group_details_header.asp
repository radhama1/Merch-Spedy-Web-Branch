...
<head>
	...
	function sortDetails(col_ordinal, sort_direction)
	{
		...
		parent.frames["GroupDetailFrame"].document.location = "security_group_details.asp?sort=" + col_ordinal + "&direction=" + sort_direction;
	}
...
<body bgcolor="999999" topmargin=0 leftmargin=0 marginheight=0 marginwidth=0 onLoad="initDataLayout('GroupDetailFrame')" oncontextmenu="return false;">
<layer name=defaultLyr id=defaultLyr><!--Netscrape 4 compatibility for header scrolling-->
<table cellpadding=0 cellspacing=0 border=0>
	<tr>
		<td><img src="./images/spacer.gif" height=1 width=5></td>
		<%=writeColHeader(0, "Group&nbsp;Name", true)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(10, "Members", true)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(3, "Group&nbsp;Summary", false)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(1, "Date&nbsp;Last&nbsp;Modified", true)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(2, "Date&nbsp;Created", true)%>
		<%=writeSeparatorBar()%>
