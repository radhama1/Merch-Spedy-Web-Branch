...
<head>
	...
	function sortDetails(col_ordinal, sort_direction)
	{
		...
		parent.frames["DetailFrame"].document.location = "validation_rules_detail.asp?sort=" + col_ordinal + "&direction=" + sort_direction;
	}
...
<table cellpadding=0 cellspacing=0 border=0>
	<tr>
		<td><img src="./images/spacer.gif" height=1 width=5></td>
		<%=writeColHeader(2, "Rule", true)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(3, "Type", true)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(4, "Column", true)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(5, "Enabled", true)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(0, "&nbsp;", false)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(6, "Modified", true)%>
		<%=writeSeparatorBar()%>
