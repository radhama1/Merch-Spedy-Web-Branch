...
SQLStr = "sp_SPEDY_TaxWizard_Questions_ShowSorting_by_ParentID '0" & Session.Value("taxID") & "', '0" & parentQuestionID & "'"
...
<table width=100% cellpadding=0 cellspacing=0 border=0>
	<form name="theForm" action="question_sort_save.asp" method="POST">
				...
				<tr>
					...
					<td valign=top>
						...
						for rowCounter = 0 to dictDetailsDataCols("RecordCount") - 1
							Tax_Question = SmartValues(arDetailsDataRows(dictDetailsDataCols("Tax_Question"), rowCounter), "CStr")
							thisID = SmartValues(arDetailsDataRows(dictDetailsDataCols("ID"), rowCounter), "CStr")
						Response.Write "<option value=""" & thisID & """>" & Tax_Question & "</option>" & vbCrLf
						next
						%>
						</select>
					</td>
			...
			<table cellpadding=0 cellspacing=0 border=0>
				<tr>
					...
					<td valign=top>
						...
						Sorry, the item you selected has nothing to sort.
						<br><br>
	...
	<input type=hidden name="pcid" value="<%=parentQuestionID%>">
	</form>
</table>
<script language="javascript">
	<!--
		parent.frames["header"].document.location = "question_sort_header.asp?pqid=<%=parentQuestionID%>";
		<%if dictDetailsDataCols("ColCount") > 0 and dictDetailsDataCols("RecordCount") > 0 then%>
		parent.frames["controls"].document.location = "question_sort_footer.asp?pqid=<%=parentQuestionID%>";
		<%end if%>
	//-->
</script>
