...
if Request.Form.Count > 0 and len(Request.Form("Category_Name")) > 0 then
	...
	objRec.Open "Website_Category", objConn, adOpenKeyset, adLockOptimistic, adCmdTable
	objRec.AddNew
	
	objRec("Website_ID") = Session.Value("websiteID")
	...
	SQLStr = "SELECT @@IDENTITY FROM Website_Category"
	Set objRec = objConn.Execute(SQLStr)
	newCatID = objRec(0)
	objRec.Close

	SQLStr = "INSERT INTO Website_SortOrder (Website_ID, Website_Category_ID) VALUES (" & Session.Value("websiteID") & ", " & newCatID & ")"
	Set objRec = objConn.Execute(SQLStr)

	objConn.CommitTrans

	Call DB_CleanUp
	Response.Redirect "website_category_save_result.asp"
end if
...
<table cellpadding=0 cellspacing=0 border=0 align=center>
	...
	<form name="theForm" action="website_category_add.asp" method=POST>
	<tr>
		<td colspan=2>
