...
'	on error resume next
	...
	if objDocument is nothing then
		...
		'	on error resume next
			...
			objRootNode.setAttribute("value") = "Configured Websites"					'Display text
			objRootNode.setAttribute("type") = "root"									'Root/Folder/Document
			objRootNode.setAttribute("webid") = 0
			objRootNode.setAttribute("url") = "./website_detailfrm_frm.asp?webid=0"
		'	objRootNode.setAttribute("url") = "./content_search.asp?cid=0"
		...
		'	on error resume next
			...
			if not objRec.EOF then
				...
				Do Until objRec.EOF
					...
					objChildNode.setAttribute("value") = objRec("Website_Name")
					objChildNode.setAttribute("type") = "folder"
					objChildNode.setAttribute("webid") = objRec("ID")
					objChildNode.setAttribute("url") = "./website_detailfrm_frm.asp?webid=" & objRec("ID")
				'	objChildNode.setAttribute("url") = "./content_search.asp?cid=" & objRec("ID")
...
'	on error resume next
	...
	Dim oNode, sAttrValue, sNodeType, sURL, sNodeName, sWebID
	Dim NODE_ELEMENT
	...
	For Each oNode In objNodes
		...
		if oNode.nodeType = NODE_ELEMENT Then
			...
			sWebID = oNode.getAttribute("webid")
...
%><td height="16"><img src="../app_images/folderlist_icons/websites/<%=fnChooseIcon(bIsLast, bIsRoot, sNodeType, bHasChildren, true)%>" width=31 height=16 border=0></td><td><img src="../app_images/folderlist_icons/websites/pixel.gif" width=2 height=1></td><td nowrap class=node><img src="../app_images/folderlist_icons/websites/pixel.gif" width=2 height=1><a href=<%=sURL%> target=DetailFrameWrapperTop onClick=objPreviousLink=fnSelectItem(this,objPreviousLink)><%=sAttrValue%></a><img src="../app_images/folderlist_icons/websites/pixel.gif" width=2 height=1></td></tr></table><%
			else  'Otherwise this is a folder
%><%=vbCrLf%><table border=0 cellspacing=0 cellpadding=0><!-- Category_ID: <%=sWebID%> --><tr onMouseOver="HoverRow(<%=sWebID%>);" onMouseOut="HoverRow(-1);" onClick="SelectRow(<%=sWebID%>);" oncontextmenu="displayMenu(); return false;"><%
...
%><td height="16"><img onclick="doChangeTree(this, arClickedElementID, arAffectedMenuItemID);" class=LEVEL<%=iElement%> src="../app_images/folderlist_icons/websites/<%=fnChooseIcon(bIsLast, bIsRoot, sNodeType, bHasChildren, bShowOpen)%>" id=<%=iElement%> width=31 height=16 border=0 name="<%=sMode%>"></td><td><img src="../app_images/folderlist_icons/websites/pixel.gif" width=2 height=1></td><td nowrap class=node><img src="../app_images/folderlist_icons/websites/pixel.gif" width=2 height=1><a href="<%=sURL%>" onMouseOver="window.status='';return true;" target=DetailFrameWrapperTop onClick="objPreviousLink=fnSelectItem(this,objPreviousLink)"><%=sAttrValue%></a><img src="../app_images/folderlist_icons/websites/pixel.gif" width=2 height=1></td></tr></table><%
...
function fnBuildLeftIndent(byval oNode, byval bIsLast, byVal sLeftIndent)
	...
	if (bIsLast = false) then
		...
		sLeftIndent = sLeftIndent & "<td><img src=""../app_images/folderlist_icons/websites/line.gif"" width=18 height=16></td>"
	else
		'Otherwise it is the last on the list so we just need to
		'display a blank space
		sLeftIndent = sLeftIndent & "<td><img src=""../app_images/folderlist_icons/websites/pixel.gif"" width=20 height=1 border=0></td>"
	end if
