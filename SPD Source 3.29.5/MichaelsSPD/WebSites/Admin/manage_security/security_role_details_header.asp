...
<head>
	...
	function sortDetails(col_ordinal, sort_direction)
	{
		...
		parent.frames["GroupDetailFrame"].document.location = "security_role_details.asp?sort=" + col_ordinal + "&direction=" + sort_direction;
	}
	//-->
	</script>
...
<table cellpadding=0 cellspacing=0 border=0>
	<tr>
		...
		<%=writeSeparatorBar()%>
		<%=writeColHeader(4, "Group&nbsp;Summary", false)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(2, "Date&nbsp;Last&nbsp;Modified", true)%>
		<%=writeSeparatorBar()%>
		<%=writeColHeader(3, "Date&nbsp;Created", true)%>
		<%=writeSeparatorBar()%>
