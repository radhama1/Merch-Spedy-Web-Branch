...
if IsNumeric(SortColumn) and Trim(SortColumn) <> "" then
	...
	Session.Value("Security_User_Group_SortColumn") = SortColumn
else
	if IsNumeric(Session.Value("Security_User_Group_SortColumn")) and Trim(Session.Value("Security_User_Group_SortColumn")) <> "" then
		SortColumn = CInt(Session.Value("Security_User_Group_SortColumn"))
	else
		SortColumn = 0
		Session.Value("Security_User_Group_SortColumn") = SortColumn
	end if
...
<head>
	...
	<script language="javascript" src="./include/security_role_details_contextmenu.js"></script><!--right click menu-->
	...
	<!--
		...
		{
			editWin = window.open("./security_role_admin/security_role_details_frm.asp?gid=" + RowID, "editWindow_" + RowID, "width=600,height=500,toolbar=0,location=0,directories=0,status=1,menuBar=0,scrollBars=1,resizable=0");
			editWin.focus();
		}
		...
		function deleteDocument(RowID)
		{
			...
			{
				document.location = "./security_role_admin/security_role_remove.asp?gid=" + RowID + "&sort=<%=SortColumn%>&direction=<%=SortDirection%>";
			}
		}
		...
		function showUserList(selectedID)
		{
			if (selectedID > 0)
			{
				parent.parent.frames['WorkspaceFrame'].document.location = 'security_role_details_frm.asp?showusers=1&grouptype=2&id=' + selectedID;
			}
		}
	//-->
	</script>
...
<table width=100% cellpadding=0 cellspacing=0 border=0>
	...
	<tr>
		<td width=100%>
			<%
			SQLStr = "sp_security_list_roles " & SortColumn & ", " & SortDirection
			...
			<script language="javascript">
			<!--
				parent.frames["DetailFrameHdr"].document.location = "security_role_details_header.asp?sort=" + <%=SortColumn%> + "&direction=" + <%=SortDirection%>;
...
<!--#include file="./include/security_role_details_contextmenu.asp"--><!--right click menu-->
