...
--***************************************************************************
ALTER TABLE SPD_Import_Items
add ReshippableInnerCartonWeight decimal(18,6) null


--***************************************************************************
--ALTER TRIGGER TRG_Stocking_Strategy_IUD
--***************************************************************************
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	begin
		...
		select @stock_group_id, t.Strategy_Code, left(t.Strategy_Desc,40), row_number() over(order by Strategy_Desc asc) 
...
/****** Object:  Trigger [dbo].[TRG_SPD_Item_Master_Vendor_Countries_IU]    Script Date: 02/09/2018 15:07:32 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
BEGIN
    ...
    set Each_Case_Cube = ins.Each_Case_Height * ins.Each_Case_Width * ins.Each_Case_Length / 1728
		, Inner_Case_Cube = ins.Inner_Case_Height * ins.Inner_Case_Width * ins.Inner_Case_Length / 1728
		, Master_Case_Cube = ins.Master_Case_Height * ins.Master_Case_Width * ins.Master_Case_Length / 1728
    from inserted ins
