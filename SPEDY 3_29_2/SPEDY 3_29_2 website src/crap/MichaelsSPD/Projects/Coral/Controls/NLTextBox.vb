...
<ToolboxData("<{0}:NLTextBox runat=""server""></{0}:NLTextBox>")> _
Public Class NLTextBox
...
#Region "INLChangeControl"
    ...
    Public ReadOnly Property ValueChanged() As Boolean Implements INLChangeControl.ValueChanged
        Get
            If Me.TreatEmptyAsZero Then
                If NLChangeControl.IsEmptyOrZero(OriginalValue) AndAlso NLChangeControl.IsEmptyOrZero(Text) Then
                    Return False
                Else
    ...
    Public Sub SetOriginalValue(ByVal value As Object, Optional ByVal setValue As Boolean = True) Implements INLChangeControl.SetOriginalValue
        OriginalValue = value
        If setValue Then Me.Text = value.ToString()
    End Sub
...
#End Region
    ...
    Protected Overrides Sub Render(ByVal writer As HtmlTextWriter)
        If Me.ChangeControl Then
            NLChangeControl.RenderBeginChangeControl(writer, Me)
        End If
        If RenderReadOnly Then
            ...
            writer.Write("&nbsp;" & Me.Page.Server.HtmlEncode(NLControlHelper.RemoveDangerousText(Me.Text)))
    ...
    Private Sub NLTextBox_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
