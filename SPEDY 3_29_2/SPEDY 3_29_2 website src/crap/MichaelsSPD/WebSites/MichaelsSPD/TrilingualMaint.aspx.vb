...
Partial Class TrilingualMaint
    Inherits MichaelsBasePage

    'PAGING
    Const cTMBATCHPERPAGE As String = "TMBATCHPERPAGE"
    Const cTMBATCHCURPAGE As String = "TMBATCHCURPAGE"
    Const cTMBATCHTOTALPAGES As String = "TMBATCHTOTALPAGES"
    Const cTMBATCHSTARTROW As String = "TMBATCHSTARTROW"
    Const cTMBATCHTOTALROWS As String = "TMBATCHTOTALROWS"

    'SORTING
    Const cTMBATCHCURSORTCOL As String = "TMBATCHCURSORTCOL"
    Const cTMBATCHCURSORTDIR As String = "TMBATCHCURSORTDIR"

    'FILTERING
    Const cTMBATCHSHOWSTAGE As String = "TMBATCHSHOWSTAGE"
    Const cTMBATCHSEARCHFILTER As String = "TMCREATIONSEARCHFILTER"
    ...
    Public Sub Initialize()
        ...
        If Not SecurityCheckHasAccess("SPD.ADVANCED", "SPD.ADVANCED.CREATETMBATCH", Session("UserID")) Then
            lnkUpload.Visible = False
        End If

        'Clear Search Filter
        Session(cTMBATCHSEARCHFILTER) = ""
    End Sub
    ...
    Private Sub PopulateFindShows()
        ...
        Dim vendorRelationWFStageID As Integer = objData.GetStageID(WorkflowType.EXTMaint, Models.WorkflowStageType.Initial)


        'Always add the Vendor Relations workflow Stage
        ddFindshowNew.Items.Add(New ListItem("Vendor Relations", vendorRelationWFStageID))

        'Only add the DBC/QA workflow Stage if the user is a DBC/QA user
        If IsAdminDBCQA() Then
            ddFindshowNew.Items.Add(New ListItem("DBC/QA", objData.GetStageID(WorkflowType.TrilingualMaint, Models.WorkflowStageType.DBC)))
        End If
        ...
        If Session(cTMBATCHSHOWSTAGE) Is Nothing Then
            ddFindshowNew.SelectedIndex = 0
        Else
            If Not ddFindshowNew.Items.FindByValue(Session(cTMBATCHSHOWSTAGE)) Is Nothing Then
                ddFindshowNew.SelectedValue = Session(cTMBATCHSHOWSTAGE)
            Else
                ddFindshowNew.SelectedIndex = 0
            End If
        End If
        Session(cTMBATCHSHOWSTAGE) = Me.ddFindshowNew.SelectedValue
    ...
    Private Sub PopulateGridView()
        ...
        Try
            Dim sql As String = "usp_SPD_GetTMBatches"
            ...
            If Helper.SmartValue(Session(cTMBATCHSEARCHFILTER), "CStr", "").Length > 0 Then
                xml.AddFilterCriteria(51, Helper.SmartValue(Session(cTMBATCHSEARCHFILTER), "CStr", ""))
            End If
           
            'Add BatchType filter, based on User Permissions
            If isVendorRelations() And Not IsAdminDBCQA() Then
                xml.AddFilterCriteria(4, "3")
            End If
            'Nak 4/10/2013: Per Michaels, DBC/QA can access Vendor Relation batches.
            'If IsAdminDBCQA() And Not isVendorRelations() Then
            '            xml.AddFilterCriteria(4, "4")
            'End If

            'Add Workflow Status Filters
            If Not Session(cTMBATCHSHOWSTAGE) Is Nothing Then
                Select Case (Session(cTMBATCHSHOWSTAGE))
                    Case "-5"
                    ...
                    Case Else
                        ...
                        xml.AddFilterCriteria(3, Session(cTMBATCHSHOWSTAGE).ToString())
                End Select
            End If

            'Add Sorting
            xml.AddSortCriteria(Session(cTMBATCHCURSORTCOL), Session(cTMBATCHCURSORTDIR))

            cmd.Parameters.Add("@xmlSortCriteria", SqlDbType.VarChar).Value = xml.GetPaginationXML().Replace("'", "''")
            cmd.Parameters.Add("@maxRows", SqlDbType.Int).Value = Helper.SmartValue(Session(cTMBATCHPERPAGE), "CInt", -1)
            cmd.Parameters.Add("@startRow", SqlDbType.Int).Value = Helper.SmartValue(Session(cTMBATCHSTARTROW), "CInt", 1)
            ...
            If dt.Rows.Count > 0 Then
                Session(cTMBATCHTOTALROWS) = Helper.SmartValue(dt.Rows(0)("totRecords"), "CStr", 0)
                Session(cTMBATCHSTARTROW) = Helper.SmartValue(dt.Rows(0)("RowNumber"), "CStr", 0)
            Else
                Session(cTMBATCHTOTALROWS) = 0
            End If

            UpdatePagingInformation()

            gvNewBatches.PageSize = Session(cTMBATCHPERPAGE)
    ...
    Private Function PublishMSMQMessage(ByVal batchId As Long, ByVal userID As Long) As Boolean
        ...
        Try
            ...
            Command.Connection = connection
            Command.CommandType = CommandType.StoredProcedure
            Command.CommandText = "usp_SPD_TrilingualMaint_PublishMQMessageByBatchID"
    ...
    Private Sub UpdatePagingInformation()
        ...
        'Set Defaults
        If Session(cTMBATCHPERPAGE) Is Nothing Then Session(cTMBATCHPERPAGE) = BATCH_PAGE_SIZE
        If Session(cTMBATCHCURPAGE) Is Nothing Then Session(cTMBATCHCURPAGE) = 1
        If Session(cTMBATCHTOTALPAGES) Is Nothing Then Session(cTMBATCHTOTALPAGES) = 0
        If Session(cTMBATCHSTARTROW) Is Nothing Then Session(cTMBATCHSTARTROW) = 1
        If Session(cTMBATCHTOTALROWS) Is Nothing Then Session(cTMBATCHTOTALROWS) = 0

        If Helper.SmartValue(Session(cTMBATCHTOTALROWS), "CInt", 0) > 0 Then

            If Helper.SmartValue(Session(cTMBATCHSTARTROW), "CInt", 0) > Helper.SmartValue(Session(cTMBATCHTOTALROWS), "CInt", 0) Then
                Session(cTMBATCHSTARTROW) = 1
            End If

            Session(cTMBATCHTOTALPAGES) = Fix(Helper.SmartValue(Session(cTMBATCHTOTALROWS), "CInt", 0) / Helper.SmartValue(Session(cTMBATCHPERPAGE), "CInt", 0))
            If (Helper.SmartValue(Session(cTMBATCHTOTALROWS), "CInt", 0) Mod Helper.SmartValue(Session(cTMBATCHPERPAGE), "CInt", 0)) <> 0 Then
                Session(cTMBATCHTOTALPAGES) = Helper.SmartValue(Session(cTMBATCHTOTALPAGES), "CInt", 0) + 1
            End If

            If Helper.SmartValue(Session(cTMBATCHCURPAGE), "CInt", 0) <= 0 OrElse Helper.SmartValue(Session(cTMBATCHCURPAGE), "CInt", 0) > Helper.SmartValue(Session(cTMBATCHTOTALPAGES), "CInt", 0) Then
                Session(cTMBATCHCURPAGE) = 1
            End If

        Else
            Session(cTMBATCHCURPAGE) = 1
            Session(cTMBATCHTOTALPAGES) = 0
            Session(cTMBATCHSTARTROW) = 1
        End If
    ...
    Private Sub UpdateSortingInformation()
        ...
        'Set Defaults
        If Session(cTMBATCHCURSORTCOL) Is Nothing Then Session(cTMBATCHCURSORTCOL) = 0
        If Session(cTMBATCHCURSORTDIR) Is Nothing Then Session(cTMBATCHCURSORTDIR) = PagingFiltering.SortDirection.Asc
    ...
    Protected Sub btnDDFFindNew_Click(sender As Object, e As EventArgs) Handles btnDDFFindNew.Click
        Try
            ...
            Session(cTMBATCHSHOWSTAGE) = ddFindshowNew.SelectedValue
            PopulateGridView()
        Catch ex As Exception
    ...
    Protected Sub gvNewBatches_RowCreated(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvNewBatches.RowCreated
        If (e.Row.RowType = DataControlRowType.Header) Then
            AddSortGlyph(gvNewBatches, e.Row, Session(cTMBATCHCURSORTCOL), PagingFiltering.GetSortDirectionString(Session(cTMBATCHCURSORTDIR)))
        End If
    End Sub
    ...
    Protected Sub gvNewBatches_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles gvNewBatches.RowCommand
        ...
        Select Case e.CommandName
            ...
            Case "Sort"
                ...
                If Session(cTMBATCHCURSORTCOL).ToString() = e.CommandArgument.ToString() Then

                    If Session(cTMBATCHCURSORTDIR) = PagingFiltering.SortDirection.Asc Then
                        Session(cTMBATCHCURSORTDIR) = PagingFiltering.SortDirection.Desc
                    Else
                        Session(cTMBATCHCURSORTDIR) = PagingFiltering.SortDirection.Asc
                    End If

                Else
                    Session(cTMBATCHCURSORTCOL) = e.CommandArgument.ToString()
                    Session(cTMBATCHCURSORTDIR) = PagingFiltering.SortDirection.Asc
                End If
            ...
            Case "Page"
                ...
                Select Case e.CommandArgument
                    Case "First"
                        ...
                        Session(cTMBATCHSTARTROW) = 1
                    Case "Prev"
                        If Session(cTMBATCHCURPAGE) > 1 Then
                            Session(cTMBATCHCURPAGE) -= 1
                            Session(cTMBATCHSTARTROW) = Session(cTMBATCHSTARTROW) - Session(cTMBATCHPERPAGE)
                        End If
                    Case "Next"
                        If Session(cTMBATCHCURPAGE) < Session(cTMBATCHTOTALPAGES) Then
                            Session(cTMBATCHCURPAGE) += 1
                            Session(cTMBATCHSTARTROW) = Session(cTMBATCHSTARTROW) + Session(cTMBATCHPERPAGE)
                        End If
                    Case "Last"
                        Session(cTMBATCHCURPAGE) = Session(cTMBATCHTOTALPAGES)
                        Session(cTMBATCHSTARTROW) = ((Session(cTMBATCHTOTALPAGES) - 1) * Session(cTMBATCHPERPAGE)) + 1
                End Select
            ...
            Case "PageGo"
                ...
                If newPageNum > 0 AndAlso newPageNum <= Session(cTMBATCHTOTALPAGES) Then

                    Session(cTMBATCHCURPAGE) = newPageNum
                    Session(cTMBATCHSTARTROW) = ((Session(cTMBATCHCURPAGE) - 1) * Session(cTMBATCHPERPAGE)) + 1
            ...
            Case "PageReset"
                ...
                If newBatchesPerPage >= 5 AndAlso newBatchesPerPage <= 50 Then
                    ...
                    Session(cTMBATCHCURPAGE) = 1
                    Session(cTMBATCHSTARTROW) = 1
                    Session(cTMBATCHPERPAGE) = newBatchesPerPage
            ...
            Case "PageFind"
                ...
                If ctrl IsNot Nothing Then
                    If Trim(ctrl.Text) <> String.Empty Then
                        Session(cTMBATCHSEARCHFILTER) = ctrl.Text.Trim
                        PopulateGridView()
                    End If
                End If
    ...
    Private Sub gvNewBatches_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvNewBatches.RowDataBound
        ...
        Select Case e.Row.RowType
            ...
            Case DataControlRowType.Pager
                ...
                ctrlPaging.text = String.Format("Page {0} of {1}", Session(cTMBATCHCURPAGE), Session(cTMBATCHTOTALPAGES))

                ctrlPaging = e.Row.FindControl("lblBatchesFound")
                ctrlPaging.text = Session(cTMBATCHTOTALROWS).ToString() & " " & ctrlPaging.text

                ctrlPaging = e.Row.FindControl("txtgotopage")
                If Helper.SmartValue(Session(cTMBATCHCURPAGE), "CInt", 0) < Helper.SmartValue(Session(cTMBATCHTOTALPAGES), "CInt", 0) Then
                    ctrlPaging.text = Helper.SmartValue(Session(cTMBATCHCURPAGE), "CInt", 0) + 1
                Else
                    ctrlPaging.text = "1"
                End If

                ctrlPaging = e.Row.FindControl("txtBatchPerPage")
                ctrlPaging.text = Helper.SmartValue(Session(cTMBATCHPERPAGE), "CInt")
