...
Partial Class POMaintenanceDetailsAddSKU
	Inherits MichaelsBasePage

#Region "Properties and Constants"

	'GENERAL
	Const cPOMAINTENANCEID As String = "POMAINTENANCEADDSKU_POID"

	'PAGING
	Const cPAGINGSKUPERPAGE As String = "POMAINTENANCEADDSKU_SKUPERPAGE"
	Const cPAGINGCURPAGE As String = "POMAINTENANCEADDSKU_CURPAGE"
	Const cPAGINGTOTALPAGES As String = "POMAINTENANCEADDSKU_TOTALPAGES"
	Const cPAGINGSTARTROW As String = "POMAINTENANCEADDSKU_STARTROW"
	Const cPAGINGTOTALROWS As String = "POMAINTENANCEADDSKU_TOTALROWS"

	'SORTING
	Const cSORTINGCURSORTCOL As String = "POMAINTENANCEADDSKU_CURSORTCOL"
	Const cSORTINGCURSORTDIR As String = "POMAINTENANCEADDSKU_CURSORTDIR"

	'SEARCH
	Const cPOCURXMLSEARCHFILTER As String = "POMAINTENANCEADDSKU_CURXMLSEARCHFILTERSTRING"

#End Region

	Private poMaintenanceRec As Models.POMaintenanceRecord
    Private poMaintenanceCacheRec As Models.POMaintenanceCacheRecord
	...
	Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
		...
		'Load PO Maintenance Record
		If Not IsPostBack Then
			Session(cPOMAINTENANCEID) = Helper.SmartValue(Request("POID"), "CLng", 0)
		End If

		LoadPOMaintenanceInfo()

		If Not poMaintenanceRec.ID.HasValue Then
	...
	Private Sub ShowMsg(ByVal strMsg As String, Optional ByVal type As String = "E")
		...
		End If
	End Sub
	Private Sub LoadPOMaintenanceInfo()
        poMaintenanceRec = Data.POMaintenanceData.GetRecord(Session(cPOMAINTENANCEID))
        poMaintenanceCacheRec = Data.POMaintenanceCacheData.GetRecord(Session("UserID"), Session(cPOMAINTENANCEID))
	End Sub
	...
	Private Sub PopulateControls()
		...
		lblBatchNumber.Text = poMaintenanceRec.BatchNumber

		If poMaintenanceRec.VendorNumber > 0 Then

			srchVendor.Text = poMaintenanceRec.VendorNumber
			vendorName.Text = poMaintenanceRec.VendorName

            Dim deptNumber As Integer = poMaintenanceCacheRec.PODepartmentID.GetValueOrDefault(0)
			Dim seasonalBasic As String = Helper.SmartValue(poMaintenanceRec.BasicSeasonal, "CStr", "")
            ...
            LoadDropdowns(deptNumber, poMaintenanceRec.BatchType, seasonalBasic, deptNumber = 0, True, enableITA)
	...
	Protected Function GetCheckedStatus(ByVal pPOContainsSKU As Boolean) As String
		Return pPOContainsSKU.ToString.ToLower()
	End Function
	Protected Function GetEnabledStatus(ByVal pPOContainsSKU As Boolean) As String
		Return (Not pPOContainsSKU).ToString.ToLower
	End Function
	...
	Protected Sub btnAddRecs_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAddRecs.Click
		...
		If poMaintenanceRec.ID.GetValueOrDefault(0) > 0 AndAlso poMaintenanceRec.VendorNumber.GetValueOrDefault(0) > 0 Then

            Dim itemExistsForPO As Boolean = Data.POMaintenanceData.CacheHasAtLeastOneSKU(poMaintenanceRec.ID, Session("UserID"))
			...
			For Each row As GridViewRow In gvSearch.Rows
				...
				If chkBox IsNot Nothing AndAlso chkBox.Checked = True AndAlso chkBox.Enabled = True Then
					...
					If Not itemExistsForPO AndAlso curDeptNo > 0 Then
                        ...
                        Data.POMaintenanceCacheData.UpdateWorkflowDepartmentID(poMaintenanceRec.ID, Session("UserID"), curDeptNo)
					...
					'Save
					Dim skuRecord As New Models.POMaintenanceSKURecord
					skuRecord.MichaelsSKU = curSKU
					skuRecord.UPC = curUPC
					Data.POMaintenanceSKUData.AddSKU(poMaintenanceRec.ID, skuRecord, Session("UserID"))
            ...
            Data.POMaintenanceCacheData.UpdatePODepartmentID(poMaintenanceRec.ID, Session("UserID"))
	...
	Private Sub PopulateGridView(Optional ByVal generateNewSearch As Boolean = False)
		...
		Try
            Dim sql As String = "PO_Maintenance_Add_SKU_Search"
			...
			If generateNewSearch OrElse Helper.SmartValue(Session(cPOCURXMLSEARCHFILTER), "CStr", "") = "" Then
				...
				'Vendor
				xml.AddFilterCriteria(0, poMaintenanceRec.VendorNumber)
				...
				If UPC.Length > 0 Then
					'UPC
					xml.AddFilterCriteria(6, poMaintenanceRec.VendorNumber)
				Else
			...
			cmd.Parameters.Add("@xmlSortCriteria", SqlDbType.VarChar).Value = xml.GetPaginationXML()
            ...
            cmd.Parameters.Add("@PO_Maintenance_ID", SqlDbType.BigInt).Value = Helper.SmartValue(Session(cPOMAINTENANCEID), "CLng", 0)
