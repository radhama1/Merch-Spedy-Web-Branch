...
'	on error resume next
	...
	if objDocument is nothing then
		...
		'	on error resume next
			...
			Dim objConn, objRootRec, SQLStr3, connstr
			Dim objRootNode, objChildNode, tempNode
			Dim websiteName
			
			Set objConn = Server.CreateObject("ADODB.Connection")
			Set objRootRec = Server.CreateObject("ADODB.RecordSet")

			connStr = Application.Value("connStr")
			objConn.Open connStr

			SQLStr3 = "SELECT Website_Name FROM Website WHERE ID = " & Session.Value("websiteID")
			Set objRootRec = objConn.Execute(SQLStr3)
			if not objRootRec.EOF then
				websiteName = objRootRec(0)
			else
				websiteName = "Website"
			end if
			objRootRec.Close
			...
			objRootNode.setAttribute("value") = websiteName								'Display text
...
'	on error resume next
	...
	if objDocument is nothing then
		...
		'	on error resume next
			...
			SQLStr = "sp_website_categories_by_parentCatID " & intRootNodeRefID & ", " & Session.Value("websiteID")
			...
			if not objRec.EOF then
				...
				Do Until objRec.EOF
					Set objChildNode = objDocument.createElement("ChildNode" & objRec("EntityID"))
					objChildNode.setAttribute("value") = objRec("Name")
					objChildNode.setAttribute("type") = "folder"
					objChildNode.setAttribute("id") = objRec("EntityID")
					objChildNode.setAttribute("url") = "javascript: setTargetItemID('" & CInt(objRec("EntityID")) & "'); void(0);"
					objChildNode.setAttribute("Topic_Type") = objRec("Topic_Type")
					objChildNode.setAttribute("Type1_FileName") = objRec("Type1_FileName")
					objRootNode.appendChild objChildNode 'Attach the new node to its parent
						
					if CBool(objRec("hasChildren")) then
						Call fnDescendTree(objRec("EntityID"), objDocument, objChildNode, objConn)
					end if
