...
if IsNumeric(selectedTab) and Trim(selectedTab) <> "" then
	...
	if IsNumeric(Session.Value("WF_STAGES_SELECTEDTAB")) and Trim(Session.Value("WF_STAGES_SELECTEDTAB")) <> "" then
		selectedTab = Session.Value("WF_STAGES_SELECTEDTAB")
	else
		selectedTab = 1
	end if
end if
Session.Value("WF_STAGES_SELECTEDTAB") = selectedTab

showDetail = CBool(checkQueryID(Request("showdetail"), 0))
Record_ID = CInt(checkQueryID(Request("tid"), 0))
...
<head>
	...
	function toggleFrameSize(boolShow)
	{
		if (boolShow == true)
		{
			resizeFrame(rowLayout, 'MainWorkflowListFrame', parent.parent.frames);
			boolShowFrameNextClick = false;
			resizeBar.src = "./images/resize_frame_3_dn.gif";
		}
		else
		{
			rowLayout = parent.parent.frames.document.getElementById('MainWorkflowListFrame').rows;
			resizeFrame('*,10', 'MainWorkflowListFrame', parent.parent.frames);
	...
	{
		document.location = "workflow_stages_header.asp?tab=" + tabID + "&showdetail=1&tid=<%=Record_ID%>";
	}
	...
	function loadDetailsScreen(tabID)
	{
		...
		{
			case 1:
				targetLocation = "workflow_stages_list_frm.asp?id=<%=Record_ID%>";
				break;
...
<%
if showDetail and Record_ID > 0 then
	SQLStr = "Select * from SPD_Workflow where workflow_ID = " & Record_ID
...
<table cellpadding=0 cellspacing=0 onSelectStart="return false" border=0 width=100%>
	...
	<tr><td bgcolor=cccccc width=100% align=center valign=top><a href="javascript: void(0);" onClick="toggleFrameSize(boolShowFrameNextClick); return false;" title="Show/Hide Details for Workflow <%=SmartValues(objRec("Workflow_Name"), "CStr")%>"><img id="resizeBar" name="resizeBar" src="./images/resize_frame_3_dn.gif" border=0></a></td></tr>
							...
							<tr bgcolor=333333>
								...
								<td>
									<div id="editpane_page_title" name="editpane_page_title" class="titleText">
									 <%=SmartValues(objRec("Workflow_Name"), "CStr")%>
									</div>
								</td>
							</tr>
						</table>
					...
					<td>
						<table width=100% cellpadding=0 cellspacing=0 border=0>
							<tr bgcolor=333333>
								...
								<td><a href="workflow_stages_header.asp?tab=1&showdetail=1&tid=<%=Record_ID%>" onClick="switchTab(1); return false;"><img name="navTab" class="navTab" id="itemlistTab" src="./images/tab_spedy_workflow<%=writeTabImgSuffix(1)%>.gif" height=12 width=100 border=0 alt="workflow stages" onMouseOver="window.status='';return true;" onMouseOut="window.status='';return true;"></a></td>
