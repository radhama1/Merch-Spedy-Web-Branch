...
if IsNumeric(selectedTab) and Trim(selectedTab) <> "" then
	...
	if IsNumeric(Session.Value("VALRULES_EDITPANE_SELECTEDTAB")) and Trim(Session.Value("VALRULES_EDITPANE_SELECTEDTAB")) <> "" then
		selectedTab = Session.Value("VALRULES_EDITPANE_SELECTEDTAB")
	else
		selectedTab = 1
	end if
end if
Session.Value("VALRULES_EDITPANE_SELECTEDTAB") = selectedTab
...
<head>
	...
	function toggleFrameSize(boolShow)
	{
		if (boolShow == true)
		{
			resizeFrame(rowLayout, 'MainListFrame', parent.parent.frames);
			boolShowFrameNextClick = false;
			resizeBar.src = "./images/resize_frame_3_dn.gif";
		}
		else
		{
			rowLayout = parent.parent.frames.document.getElementById('MainListFrame').rows;
			resizeFrame('*,10', 'MainListFrame', parent.parent.frames);
	...
	{
		document.location = "validation_rules_header.asp?tab=" + tabID + "&showdetail=1&tid=<%=Record_ID%>";
	}
	...
	function loadDetailsScreen(tabID)
	{
		...
		{
			case 1:
				targetLocation = "validation_rules_detail_frm.asp?tid=<%=Record_ID%>";
				break;
...
<table cellpadding=0 cellspacing=0 onSelectStart="return false" border=0 width=100%>
	...
	<tr><td bgcolor=cccccc width=100% align=center valign=top><a href="javascript: void(0);" onClick="toggleFrameSize(boolShowFrameNextClick); return false;" title="Show/Hide Details for Validation Document <%=SmartValues(objRec("Validation_Document"), "CStr")%>"><img id="resizeBar" name="resizeBar" src="./images/resize_frame_3_dn.gif" border=0></a></td></tr>
						...
						<table cellpadding=0 cellspacing=0 border=0>
							<tr bgcolor=333333>
								...
								<td>
									...
									Workflow: <%=SmartValues(objRec("Workflow_Name"), "CStr")%> &nbsp;-&nbsp; Document: &nbsp;<%=SmartValues(objRec("Validation_Document"), "CStr")%>
									</div>
								</td>
							</tr>
						</table>
					...
					<td>
						<table width=100% cellpadding=0 cellspacing=0 border=0>
							<tr bgcolor=333333>
								...
								<td><a href="validation_rules_header.asp?tab=1&showdetail=1&tid=<%=Record_ID%>" onClick="switchTab(1); return false;"><img name="navTab" class="navTab" id="itemlistTab" src="./images/tab_rules<%=writeTabImgSuffix(1)%>.gif" height=12 width=100 border=0 alt="validation rules" onMouseOver="window.status='';return true;" onMouseOut="window.status='';return true;"></a></td>
