...
if IsNumeric(SortColumn) and Trim(SortColumn) <> "" then
	...
	Session.Value("WF_List_SortColumn") = SortColumn
else
	if IsNumeric(Session.Value("WF_List_SortColumn")) and Trim(Session.Value("WF_List_SortColumn")) <> "" then
		SortColumn = CInt(Session.Value("WF_List_SortColumn"))
	else
		SortColumn = 0
		Session.Value("WF_List_SortColumn") = SortColumn
	end if
...
if IsNumeric(SortDirection) and Trim(SortDirection) <> "" then
	...
	Session.Value("WF_List_SortDirection") = SortDirection
else
	if IsNumeric(Session.Value("WF_List_SortDirection")) and Trim(Session.Value("WF_List_SortDirection")) <> "" then
		SortDirection = CInt(Session.Value("WF_List_SortDirection"))
	else
		SortDirection = 0
		Session.Value("WF_List_SortDirection") = SortDirection
	end if
...
SQLStr = "sp_SPD2_Workflow_ReturnAll " & SortColumn & ", " & SortDirection & ", " & pageSize & ", " & startRow
...
<head>
	...
	<script language="javascript" src="./include/workflow_list_contextmenu.js"></script><!--right click menu-->
	...
	<!--
		...
		function showDetailsFrame(itemID)
		{
			...
			parent.parent.frames['WorkflowDetailFrame'].document.location = "workflow_stages_frm.asp?tid="+itemID;
			if (parent.parent.frames.document.getElementById('MainWorkflowListFrame').rows == '*,0')
			{
				resizeFrame('200,*', 'MainWorkflowListFrame', parent.parent.frames);
			}
		}
		...
		{
			resizeFrame('*,0', 'MainWorkflowListFrame', parent.parent.frames);
		...
		<td width=100%>
			...
			<table cellpadding=0 cellspacing=0 onSelectStart="return false" width=100% border=0>
				...
				for rowCounter = 0 to displayCount - 1
					...
					Record_ID = SmartValues(arRecRows(dictRecCols("Workflow_ID"), rowCounter), "CInt")
				...
				<tr bgcolor=<%=rowcolor%> id="datarow_<%=Record_ID%>" onMouseOver="HoverRow(<%=Record_ID%>);" onMouseOut="HoverRow(0);" onDblClick="highlightRow();" oncontextmenu="HoverRow(<%=Record_ID%>);SelectRow(<%=Record_ID%>);displayMenu(); return false;">
					...
					<td valign=top nowrap><font style="font-family:Arial, Helvetica;font-size:11px;"><a href="javascript:showDetailsFrame(<%=Record_ID%>); void(0);"><%=SmartValues(arRecRows(dictRecCols("Workflow_Name"), rowCounter), "CStr")%></a></font></td>
					...
					<td><img src="./images/spacer.gif" height=1 width=10></td>
					<td valign=top nowrap><font style="font-family:Arial, Helvetica;font-size:11px;"><%=SmartValues(arRecRows(dictRecCols("Workflow_Description"), rowCounter), "CStr")%></font></td>
					<td><img src="./images/spacer.gif" height=1 width=10></td>
			...
			<!--
				...
				parent.frames["DetailFrameHdr"].document.location = "workflow_list_header.asp?sort=" + <%=SortColumn%> + "&direction=" + <%=SortDirection%>;
...
<!--#include file="./include/workflow_list_contextmenu.asp"--><!--right click menu-->
</body>
</html>
<%
