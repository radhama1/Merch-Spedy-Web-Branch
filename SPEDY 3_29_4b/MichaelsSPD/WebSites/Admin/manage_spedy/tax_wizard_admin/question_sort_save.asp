...
Dim parentQuestionID, sortedItems, arSortedItems, thisItem
...
parentQuestionID = Trim(Request.Form("pqid"))
...
for each thisItem in arSortedItems
	if IsNumeric(thisItem) then
		if thisItem > 0 then
			SQLStr = "SELECT SortOrder FROM SPD_Tax_Question WHERE [ID] = '0" & thisItem & "'"
