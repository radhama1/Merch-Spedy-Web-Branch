...
Class cls_Security_User_Group
	...
	Public Function Load(p_ID)
		...
		SQLStr = "SELECT * FROM Security_User_Group WHERE ID = '0" & CLng(p_ID) & "'"
		...
		Select Case rs.recordcount
			Case 1
				...
				Load = m_ID
			Case -1, 0
				err.raise 2, "cls_Security_User_Group", "Error loading class cls_Security_User_Group [Load]: Item was not found."
			Case else
				err.raise 3, "cls_Security_User_Group", "Error loading class cls_Security_User_Group [Load]: Item was not unique."			
		end Select
	End Function
...
'
'		if tempxmldoc.selectNodes("//Security_User_Group[@ID = '" & CStr(p_ID) & "']").length > 0 then
'			for each tempxmlattribute in tempxmldoc.selectSingleNode("//Security_User_Group[@ID = '" & CStr(p_ID) & "']").attributes
...
'	End Function
    ...
    Public Function Save()
		...
		SQLStr = "sp_Security_User_Group_InsertUpdate " & _
	...
	Public Function Delete()
		Dim SQLStr
        SQLStr = "DELETE FROM Security_User_Group WHERE [ID] = '0" & CLng(p_ID) & "'"
		utils.RunSQL SQLStr
	End Function
