...
function ediTable(strTblID, strTblName, intCellPadding, intCellSpacing, intBorderWidth, intTblWidth, strClassName, strStyleStr)
{
	...
	this.editBehavior = new ediTable_editBehavior();
...
function ediTable_editBehavior()
{
	this.allowAdd = false;
...
//	this.validationFunction = function(){ediTable_validateData(this)};
	...
	this.hiddenElementNamePrefix = "fld_";	
	...
	this.addBtn_align = "";
	this.addBtn_vAlign = "";
	this.removeBtn_id_prefix = "btnRemove_";
		...
		if (tblObj.Columns[j].showDataEntryField)
		{
			switch(tblObj.Columns[j].datatype)
			{
				case "boolean":
					...
					{
						document.all['new_' + tblObj.Columns[j].id].defaultChecked = true;
						document.all['new_' + tblObj.Columns[j].id].checked = true;
					}
					else
					{
						document.all['new_' + tblObj.Columns[j].id].defaultChecked = false;
						document.all['new_' + tblObj.Columns[j].id].checked = false;
					}
					break;
				case "select":
					document.all['new_' + tblObj.Columns[j].id].selectedIndex = tblObj.Columns[j].editCell_select_defaultOption;
					break;
				default:
					document.all['new_' + tblObj.Columns[j].id].value = tblObj.Columns[j].editCell_defaultValue;
					break;
			}
		}
	}
...
function ediTable_returnTable(tblObj)
{
	...
	for (var i = 0; i < tblObj.Rows.length; i++)
	{
		...
		for (var j = 0; j < tblObj.Columns.length; j++)
		{
			...
			switch (tblObj.Columns[j].datatype)
			{
				...
				//	oElem = document.createElement("INPUT");
				//	oElem.type = "text";
				//	oElem.id = "text" + tblObj.Columns[j].id + "_" + i + tblObj.editBehavior.hiddenElementNameSuffix;
				//	oElem.value = newRowVal;
				//	oElem.setAttribute("name", oElem.id, 0);
				//	oCell.insertAdjacentElement('beforeEnd', oElem);
				//	break;
	...
	for (var i = 0; i < tblObj.Columns.length; i++)
	{
		...
		if (tblObj.Columns[i].showDataEntryField == true && tblObj.editBehavior.allowAdd == true)
		{
			...
			{
				case "string":
					oCell.innerHTML = '<input type="text" name="new_' + tblObj.Columns[i].id + '" id="new_' + tblObj.Columns[i].id + '" value="' + tblObj.Columns[i].editCell_defaultValue + '" maxlength=' + tblObj.Columns[i].editCell_maxLength + ' class="' + tblObj.Columns[i].editCell_className + '" style="' + tblObj.Columns[i].editCell_styleStr + '">';
					break;

				case "number":
					oCell.innerHTML = '<input type="text" name="new_' + tblObj.Columns[i].id + '" id="new_' + tblObj.Columns[i].id + '" value="' + tblObj.Columns[i].editCell_defaultValue + '" maxlength=' + tblObj.Columns[i].editCell_maxLength + ' class="' + tblObj.Columns[i].editCell_className + '" style="text-align:right;' + tblObj.Columns[i].editCell_styleStr + '">';
					break;

				case "date":
					//	oCell.innerHTML = '<table cellspacing=0 cellpadding=0 border=0><tr><td><input type="text" name="new_' + tblObj.Columns[i].id + '" id="new_' + tblObj.Columns[i].id + '" value="' + tblObj.Columns[i].editCell_defaultValue + '" maxlength=10 class="' + tblObj.Columns[i].editCell_className + '" style="width:70px;' + tblObj.Columns[i].editCell_styleStr + '"></td><td><a href="javascript:ediTable_dateWin(\'new_' + tblObj.Columns[i].id + '\');"><img src="../../app_images/mini_calendar.gif" border=0 alt="Click here to select your date from a calendar"></a></td></tr></table>';
					...
					nElem.type = "text";
					nElem.id = "new_" + tblObj.Columns[i].id;
					...
					var tempStr = "new_" + tblObj.Columns[i].id + "";
				...
				case "boolean":
					...
					oCell.innerHTML = '<input type="checkbox" ' + defaultCheckedStr + ' name="new_' + tblObj.Columns[i].id + '" id="new_' + tblObj.Columns[i].id + '" value="' + tblObj.Columns[i].editCell_defaultValue + '" class="' + tblObj.Columns[i].editCell_className + '" style="border: 0px; ' + tblObj.Columns[i].editCell_styleStr + '">&nbsp;' + tblObj.Columns[i].defaultValue;
					break;
				...
				case "multiline-text":
					oCell.innerHTML = '<textarea name="new_' + tblObj.Columns[i].id + '" id="new_' + tblObj.Columns[i].id + '" class="' + tblObj.Columns[i].editCell_className + '" style="' + tblObj.Columns[i].editCell_styleStr + '">' + tblObj.Columns[i].editCell_defaultValue + '</textarea>';
					break;
...
function ediTable_insertRow(tblObj)
{
	...
	{
		oCell = document.createElement("TD");
		newRowVal = (typeof(document.all["new_" + tblObj.Columns[j].id]) == "object") ? document.all["new_" + tblObj.Columns[j].id].value : tblObj.Columns[j].defaultValue;
		newRowVal = (newRowVal != "") ? newRowVal : "&nbsp;";
	...
	//	}
		switch (tblObj.Columns[j].datatype)
		{
			...
			case "select":
				var newRowText = document.all["new_" + tblObj.Columns[j].id][document.all["new_" + tblObj.Columns[j].id].selectedIndex].text;
				newRowVal = document.all["new_" + tblObj.Columns[j].id].value;
			...
			case "boolean":
				...
				newRowVal = (document.all['new_' + tblObj.Columns[j].id].checked == true) ? boolStr_PositiveResponse : boolStr_NegativeResponse;
	...
	ediTable_updateDebugWindow(tblObj);
	ediTable_clearDataEntryFields(tblObj);
	document.all["new_" + tblObj.Columns[0].id].focus();
//	alert(msg);
}
