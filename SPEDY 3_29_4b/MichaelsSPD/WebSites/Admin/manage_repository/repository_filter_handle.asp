...
<head>
	...
	<script language="javascript">
		...
		var treeFrameMinWidth			= 0;							// desired minimum width of tree frame when closed
		var treeFrameMaxWidth			= 200;							// desired maximum width of tree frame when opened
		var treeFramesetPath			= parent.frames;				// path up to frameset
		var treeFramesetName			= "MainDetailFrameWrapper";		// id assigned to frameset
		var treeFramesetType			= "cols";						// rows or cols
		var treeDimensionsPrefixString	= "";							// string to precede the tree frame dimension
		var treeDimensionsSuffixString	= ",*";							// string to follow the tree frame dimension
		...
		var ParentFramesetName			= "RepositoryWrapperFrameset";	// id assigned to frameset
		var FramesetType				= "rows";						// rows or cols
		var DimensionsPrefixString		= "";							// string to precede the filter frame dimension
		...
		function openFilterFrame()
		{
			if (useEffects)
			{
				var treeFilterFrameTween = new Tween(new Object(),'',Tween.backEaseIn,treeFrameMaxWidth,treeFrameMinWidth,0.5);
				treeFilterFrameTween.onMotionChanged = function(event){
					resizeFrame(treeDimensionsPrefixString + Math.round(event.target._pos) + treeDimensionsSuffixString, treeFramesetName, treeFramesetPath, treeFramesetType);
				};
				treeFilterFrameTween.start();
		...
		function closeFilterFrame()
		{
			if (useEffects)
			{
				var treeFilterFrameTween = new Tween(new Object(),'',Tween.backEaseOut,treeFrameMinWidth,treeFrameMaxWidth,0.5);
				treeFilterFrameTween.onMotionChanged = function(event){
					resizeFrame(treeDimensionsPrefixString + Math.round(event.target._pos) + treeDimensionsSuffixString, treeFramesetName, treeFramesetPath, treeFramesetType);
				};
				treeFilterFrameTween.start();
