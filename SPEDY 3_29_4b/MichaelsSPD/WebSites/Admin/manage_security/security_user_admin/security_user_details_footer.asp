	...
	<!--
		...
		function saveOnly()
		{
			if (confirm("Really apply these changes?\n\nThis cannot be undone."))
			{
				...
				parent.frames['body'].document.theForm.action = "security_user_details_work_noclose.asp";
				parent.frames['body'].document.theForm.target = "calcFrame";
				var success = parent.frames['body'].doSubmit();

                if (success == true) {
				    //put everything back the way e found it...
				    parent.frames['body'].document.theForm.target = "body";
				    parent.frames['body'].document.theForm.action = "security_user_details_work.asp";
