...
function SearchByStore(storeNumber) {
    ...
    for (var i = 0; i < rowCollection.length; i++) {
        ...
        if (cellCollection[1].children.length > 0) {
            num = cellCollection[1].children[0].innerHTML;
        }
        else {
            num = cellCollection[1].innerHTML;
        }
        if (storeNumber == num) {
            var element = document.getElementById("SKUStoreGrid").rows[i]
            element.style.backgroundColor = "#F6F6C6";
            cellCollection[1].scrollIntoView();
            cellCollection[1].focus();
        }
        else {
            var element = document.getElementById("SKUStoreGrid").rows[i]
